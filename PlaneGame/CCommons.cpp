...
void CCommons::convertFileNameToLonLat(const QString &name, double *lon, double *lat)
{
    ...
    fnLat.replace(2, 1, '.');  //Here there should have been a point and not a coma 
    fnLon.replace(3, 1, '.');  //because otherwise it breakes toDouble() function few lines futher
...
void CCommons::stringIntoVSConsole(char* message) {
    OutputDebugStringA(message);
}

void CCommons::doubleIntoVSConsole(double number) {
    char buffer[64];
