...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(  ), ui(new Ui::MainWindow)
{
    ...
    CCamera *camera;
    ui->setupUi(this);
    int mogetonazwacjakchceibedziedzialalo = 9;
    QGLFormat glFormat;
    glFormat.setDoubleBuffer(true);     //Set double buffer for speed??
    glFormat.setSampleBuffers(true);    //Turns on multisampling to provide antyaliasing 
    glFormat.setAlpha(true);            //Apha channel used to control opacity
    glFormat.setDepthBufferSize(24);    
    openGl = new COpenGl(glFormat, centralWidget());
    centralWidget()->layout()->addWidget(openGl);       //deploing a new widget 
