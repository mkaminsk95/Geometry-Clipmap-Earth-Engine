...
void CTerrain::findTerrainPointClosestToCam()
{
    ...
    if (terrainPointClosestToCamDistance < earth->orientationPointDistance)
    {
        earth->orientationPointDistance = terrainPointClosestToCamDistance;
        earth->orientationPoint.setX(terrainPointClosestToCam->x());
        earth->orientationPoint.setY(terrainPointClosestToCam->y());
        earth->orientationPoint.setZ(terrainPointClosestToCam->z());

        
        earth->h[0] = terrainData->getHeight(0, 0)[0];
        earth->h[1] = terrainData->getHeight(8, 0)[0];
        earth->h[2] = terrainData->getHeight(0, 8)[0];
        earth->h[3] = terrainData->getHeight(8, 8)[0];
        //earth->h[1] = terrainData->getHeight(0, 0);
      /*char buffer[64];
        sprintf(buffer, "%g", terrainpointclosesttocam->x);
        sprintf(buffer, "%g", terrainpointclosesttocam->y);
        sprintf(buffer, "%g", terrainpointclosesttocam->z);
...
bool CTerrain::draw()
{
    ...
    for (i=0; i<4; i++) {
        ...
        if (!childDrawed  && terrainInCameraFOV) {
            ...
            if (dss->drawTerrainPoint)              terrainData->drawPoint(xStart, xStop, yStart, yStop, dss);          //drawing vertices
            if (dss->drawTerrainWire)               terrainData->drawWire(xStart, xStop, yStart, yStop, dss);           //drawing wire (just squares not triangles)
            if (dss->drawTerrainBottomPlaneWire)    terrainData->drawBottomPlaneWire(xStart, yStart, dss);              //drawing planewire (tiles)
            if (dss->drawTerrainSolid)            { if (dss->drawTerrainSolidStrip)                                     //drawing colored tiles
            ...
            if (dss->drawTerrainTexture)          { if (dss->drawTerrainTextureStrip)                                   //drawing terrain textures
            ...
            if (dss->drawTerrainNormals)            terrainData->drawNormals(xStart, xStop, yStart, yStop, dss);        //drawing normals
        }
    }
